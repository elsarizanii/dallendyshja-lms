...
<body class="h-full font-sans antialiased text-zinc-900">
    ...
    <div class="flex h-screen overflow-hidden flex-col md:flex-row">
                ...
                <div class="border-t border-slate-800 bg-slate-950/40 p-4 space-y-2">
                    ...
                    <div class="flex items-center justify-between bg-slate-900/60 rounded-lg p-2.5 border border-slate-800/60">
                        <div class="flex items-center min-w-0">
                            @auth
                                <div class="h-8 w-8 rounded-full bg-blue-600 flex items-center justify-center text-white font-semibold text-xs shrink-0 uppercase">
                            ...
                            @else
                                <div class="h-8 w-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-500 text-xs shrink-0">G</div>
                                <div class="ml-2.5 min-w-0">
                                    <p class="text-xs font-semibold text-slate-400">Mysafir</p>
                                </div>
                            @endauth
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between bg-slate-900/60 rounded-lg p-2.5 border border-slate-800/60">
                    <div class="flex items-center min-w-0">
                        @auth
                            ...
                            <div class="ml-2.5 min-w-0">
                                <p class="text-xs font-semibold text-slate-200 truncate">{{ auth()->user()->name }}</p>
                                <p class="text-[10px] text-slate-500 font-mono truncate">ID: #{{ auth()->id() }}</p>
                            </div>
                        @else
                            <div class="h-8 w-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-500 font-semibold text-xs shrink-0">
                                G
                            </div>
                            <div class="ml-2.5 min-w-0">
                                <p class="text-xs font-semibold text-slate-400 truncate">Mysafir</p>
                                <p class="text-[10px] text-slate-600 font-mono truncate">Ju lutemi kyçuni</p>
                            </div>
                        @endauth
                    </div>
                    @auth
                        <a href="/profile" class="p-1.5 rounded-md text-slate-500 hover:text-slate-300 hover:bg-slate-800 transition-colors">
        ...
        <div class="flex flex-col flex-1 w-0 overflow-hidden">
            <header class="h-16 bg-white border-b border-zinc-200 flex items-center px-6 justify-between shadow-sm shrink-0">
                ...
                <!-- Dynamic URL Segments Breadcrumb -->
                <div class="text-xs font-semibold text-zinc-400 tracking-wider flex items-center space-x-1 uppercase">
                    <span>Portal</span>
                    @foreach(request()->segments() as $segment)
                        @if($segment !== 'portal')
                            <span class="text-zinc-300 font-normal">/</span>
                            <span class="text-zinc-600 font-medium last:text-zinc-800">
                                {{ str_replace('-', ' ', $segment) }}
                            </span>
                        @endif
                    @endforeach
                ...
                <div class="flex items-center space-x-4 h-full">
                    <a href="/chat" class="p-2 text-zinc-400 hover:text-zinc-600 rounded-full hover:bg-zinc-100 transition-all flex items-center justify-center">
                    ...
                    <button class="p-2 text-zinc-400 hover:text-zinc-600 rounded-full hover:bg-zinc-100 transition-all relative flex items-center justify-center">
                    ...
                    <div class="h-6 w-px bg-zinc-200 self-center"></div>
                    
                    <div class="flex items-center h-full">
                        <!-- Authenticated Header User Profile Section -->
                        @auth
                            <div class="flex flex-col items-end justify-center font-sans">
                                <span class="font-semibold text-zinc-800 text-sm leading-none">
                                    {{ auth()->user()->name }}
                                </span>
                                <span class="text-[10px] font-bold text-zinc-500 bg-zinc-100/80 border border-zinc-200/80 px-1.5 py-0.5 rounded mt-1 tracking-wider uppercase">
    ...
    <!-- Mobile Navigation Panel Logic Script -->
    <script>
